...
case class VarDeclStmt(bindings: List[HostLanguageNode]) extends Statement
...
case class SwitchStmt(head: Expression, cases: List[HostLanguageNode]) extends Statement
...
case class TryStmt(body: Statement, catchBlock: Option[HostLanguageNode], finallyBlock: Option[HostLanguageNode]) extends Statement
...
case class ObjectExpr(bindings: List[HostLanguageNode]) extends Expression
...
case class FunctionDecl(name: Literal, args: List[Literal], body: Statement) extends Expression
case class FunctionExpr(args: List[Literal], body: Statement) extends Expression
